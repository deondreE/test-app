...
<main class="">
  ...
  <div class="block">
    ...
    <div class="flex items-start space-x-4 p-4 rounded-lg bg-white mb-4 overflow-hidden max-w-3xl">
      <!-- Left side: Username and Time on the same line -->
      <div class="flex flex-col flex-grow">
        <div class="flex justify-between w-full">
          <span class="font-semibold text-gray-800 text-lg">{user.username}</span>
          <span class="text-sm text-gray-500">{user.time}</span>
        </div>
        <!-- Message underneath the name and time -->
        <span class="whitespace-pre-line text-lg font-medium text-gray-800 mt-2">{user.message}</span>
      </div>
    </div>
  ...
  <div class="absolute inset-x-0 bottom-0 p-4">
    <textarea
      bind:value={message}
      class="ml-[200px] w-[30rem] h-32 p-4 text-lg border-2 border-gray-300 rounded-md shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
